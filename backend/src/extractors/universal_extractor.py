...
19. SPECIALTY RX DESCRIPTION: This field captures specialty drug costs from the In-Network column ONLY. Look for the "Specialty drugs" row in the pharmacy table (usually after Tier 3). Extract the complete In-Network value including all information. Include BOTH percentages AND dollar amounts if both appear. Examples: If In-Network shows "30% (preferred), 50% (non-preferred)", extract as "30%/50%". If it shows "$300/$500", extract as-is. If it shows "Applicable cost as noted above for generic or brand drugs", "Same as above", or "Matches Previous Tiers", extract that exact phrase into specialty_rx_description AND set specialty_mirrors_tiers_1_3 to true. If no specialty drugs row exists, set to null. CRITICAL: Extract ONLY from In-Network column, NEVER from Out-of-Network or Limitations columns. NEVER put specialty cost into tier_4_copay or tier_5_copay fields - it belongs ONLY in specialty_rx_description. Do not overwrite or remove existing copay/coinsurance values from Tiers 1-5; they must coexist with the specialty drug values.
